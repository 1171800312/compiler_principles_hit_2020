...
public class Smantic {
	...
	public Smantic(String filename, List<FourAddr> four_addr) {
		Smantic.four_addr = four_addr;
		analyze(filename);
	}
	...
	public static void semantic_6(Tree tree) {
		// 这里X是基本类型，C是下标序列
		int T = tree.getFather().getId(); // T节点的唯一标识符
	...
	public static void semantic_23(Tree tree) {
		...
		if (s.contains("array")) {
			...
			four_addr.add(new FourAddr("*", tree_pro.get(E).getAddr(), String.valueOf(util.typeWidth(s)), newtemp));
		} else {
	...
	public static void semantic_24(Tree tree) {
		...
		if (s.contains("array")) {
			...
			four_addr.add(new FourAddr("*", tree_pro.get(E).getAddr(), String.valueOf(util.typeWidth(s)), newtemp1));
		} else {
